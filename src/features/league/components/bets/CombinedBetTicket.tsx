        ...
        <div className="flex flex-wrap items-center justify-between gap-x-4 gap-y-3 md:flex-col md:items-end md:justify-center">
          ...
          <div className="text-left md:text-right">
            <div className="text-xs text-neutral-500 dark:text-neutral-400 uppercase font-semibold">
              {bet.status === 'CASHOUT' ? 'Cashout Retirado' : `Ganancia Potencial (x${bet.total_odds.toFixed(2)})`}
            </div>
            <div className={`font-black ${bet.status === 'CASHOUT' ? 'text-blue-600 dark:text-blue-400' : 'text-emerald-600 dark:text-emerald-400'}`}>
              S/. {bet.status === 'CASHOUT' ? (bet.cashout_value || 0).toFixed(2) : bet.potential_win.toFixed(2)}
            </div>
          </div>
